...
window.onload = function () {
    ...
    if (searchParams.has('t')) {
        ...
        document.cookie = `affiliate_tracking=${searchParams.get('t')};domain=${getDomain()}; path=/;`;
    }
    const signup_device_cookie_unparsed = getCookie('signup_device') || '{}';
    const signup_device_cookie = JSON.parse(decodeURI(signup_device_cookie_unparsed).replaceAll('%2C', ','));
    ...
    const date_first_contact_cookie = JSON.parse(decodeURI(date_first_contact_cookie_unparsed).replaceAll('%2C', ','));
    if (!date_first_contact_cookie.date_first_contact) {
        const ws = new WebSocket(`wss://green.binaryws.com/websockets/v3?app_id=${app_id}&brand=${brand_name}`);
        ...
        ws.onmessage = function (msg) {
            ...
            const date_first_contact_data_cookie = encodeURI(JSON.stringify(date_first_contact_data))
                .replace(',', '%2C')
                .replace('%7B', '{')
                .replace('%7D', '}');
